...
function RendezVous() {
  ...
  const lateAlertsCount = filteredAlerts.filter(a => a.status === 'late').length;

  const handleUpdateStatus = async (alertId: string, status: Alert['status']) => {
    try {
      await updateAlertStatus(alertId, status);
      
      // Trouver l'alerte pour obtenir l'appointmentId
      const alert = alerts.find(a => a.id === alertId);
      if (alert?.appointmentId) {
        // Mettre à jour également le statut du rendez-vous
        await updateAppointmentStatus(alert.appointmentId, status);
      }
    } catch (error) {
          ...
          <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow-sm`}>
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex gap-2">
                  <span className={`px-2.5 py-1 ${darkMode ? 'bg-orange-900 text-orange-300' : 'bg-orange-100 text-orange-700'} rounded-full text-sm`}>
                    {pendingAlertsCount} en attente
                  </span>
                  <span className={`px-2.5 py-1 ${darkMode ? 'bg-red-900 text-red-300' : 'bg-red-100 text-red-700'} rounded-full text-sm`}>
