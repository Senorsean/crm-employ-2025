...
function Layout() {
  ...
  useEffect(() => {
    ...
    // Add Monitoring for admin, analyst roles, and slucas@anthea-rh.com
    if (userData?.role === 'admin' || userData?.role === 'analyst' || userData?.email === 'slucas@anthea-rh.com') {
  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'dark bg-gray-900 text-white' : 'bg-gray-50 text-gray-900'}`}>
      {/* Mobile Header */}
      <div className={`lg:hidden fixed top-0 left-0 right-0 h-16 ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow-sm z-50 flex items-center px-4`}>
      ...
      >
        ...
        <div className={`hidden lg:flex flex-col p-6 border-b ${darkMode ? 'border-gray-700' : 'border-anthea-lime'} ${isCollapsed ? 'items-center justify-center' : 'items-center'}`}>
          {!isCollapsed ? (
            ...
            // Logo rétréci - menu replié - Supprimé comme demandé
